...
export class BossanovaMissionViewComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.apiService.getStore(storeId, startDate, new Date()).subscribe(store => {
      this.store = store;
      this.apiService.getMission(store.storeId, missionId, Intl.DateTimeFormat().resolvedOptions().timeZone).subscribe(mission => {
        this.titleService.setTitle(mission.storeName + ' - ' + mission.missionName);
        this.mission = mission;
        this.aisles = mission.aisles;
          // If this page was nagivated to from the store view, show the two week average from there, if not show the last two weeks average
